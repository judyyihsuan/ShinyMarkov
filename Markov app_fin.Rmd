...
tabPanel("probability of (servere) exacerbation", 
         fluidRow(
           column(3,
        numericInput(inputId = "SI_p_a_E_MA",      
                   label = "probability of exacerbation_trtA_mild", 
                   value = 0.05,              
                   min = 0,                   
                   max = 1),

        numericInput(inputId = "SI_p_a_E_MB",      
                   label = "probability of exacerbation_trtA_moderate", 
                   value = 0.08,              
                   min = 0,                   
                   max = 1),

        numericInput(inputId = "SI_p_a_E_S",      
                   label = "probability of exacerbation_trtA_servere", 
                   value = 0.11,              
                   min = 0,                   
                   max = 1)
           ),
        ...
        numericInput(inputId = "SI_p_b_E_MA",      
                   label = "probability of exacerbation_trtB_mild", 
                   value = 0.08,              
                   min = 0,                   
                   max = 1),

        numericInput(inputId = "SI_p_b_E_MB",      
                   label = "probability of exacerbation_trtB_moderate", 
                   value = 0.1,              
                   min = 0,                   
                   max = 1),

        numericInput(inputId = "SI_p_b_E_S",      
                   label = "probability of exacerbation_trtB_servere", 
                   value = 0.12,              
                   min = 0,                   
                   max = 1)
        ),
        ...
        numericInput(inputId = "SI_p_a_SE_MA",      
                   label = "probability of servere exacerbation_trtA_mild", 
                   value = 0.1,              
                   min = 0,                   
                   max = 1),

        numericInput(inputId = "SI_p_a_SE_MB",      
                   label = "probability of servere exacerbation_trtA_moderate", 
                   value = 0.15,              
                   min = 0,                   
                   max = 1),

        numericInput(inputId = "SI_p_a_SE_S",      
                   label = "probability of servere exacerbation_trtA_servere", 
                   value = 0.2,              
                   min = 0,                   
                   max = 1)
        ),
    column(3,
        numericInput(inputId = "SI_p_b_SE_MA",      
                   label = "probability of servere exacerbation_trtB_mild", 
                   value = 0.15,              
                   min = 0,                   
                   max = 1),

        numericInput(inputId = "SI_p_b_SE_MB",      
                   label = "probability of servere exacerbation_trtB_moderate", 
                   value = 0.2,              
                   min = 0,                   
                   max = 1),

        numericInput(inputId = "SI_p_b_SE_S",      
                   label = "probability of servere exacerbation_trtB_servere", 
                   value = 0.25,              
                   min = 0,                   
                   max = 1)))), 
  ...
  tabPanel("maintain care_resource use" ,
            fluidRow(
    column(4,
 ...
 column(4,      
       numericInput(inputId = "SI_ru_MB_gp",     
                   label = "number of plum visit for moderate patients per year",       
                   value = 3,              
                   min = 0,                   
                   max = 10), 
 ...
 column(4, 
    ...
    numericInput(inputId = "SI_ru_S_gp",     
                   label = "number of pulm visit for servere patients per year",       
                   value = 4,              
                   min = 0,                   
                   max = 10), 
 ...
 tabPanel("maintain care_unit cost" ,
            fluidRow(
    column(6,
    ...
    numericInput(inputId = "SI_uc_theoph",     
      ...
      numericInput(inputId = "SI_uc_outspecialist",     
                   label = "unit cost of pulm specialist visit",       
                   value = 60,              
                   min = 10,                   
                   max = 120)   
  ...
  tabPanel("Treatment for exacerbation_resource use" ,
        fluidRow(
    column(3,     
...
column(3,
 ...
 tabPanel("Treatment for (servere) exacerbation_unit cost" ,
        fluidRow(
    column(6,     
 ...
 tabPanel(
   "select results to show" , 
    ...
    mainPanel(                                
      ...
      tabPanel("Transition Probability for treatment A",fluidRow(tableOutput(outputId="prob_table_trtA_first"),tableOutput(outputId="prob_table_trtA_after"))),
      tabPanel("Transition Probability for treatment B",fluidRow(tableOutput(outputId="prob_table_trtB_first"),tableOutput(outputId="prob_table_trtB_after")))
...
server <- function(input, output){   
 ...
 datasetinput<- reactive({
                   ...
                   ppp<- {if (country== 2){ppp<- 0.83}
                   else   if (country== 3){ppp<- 1.25}
                   else {ppp<-1}}
                   ...
                   p_a_MA <- input$SI_p_a_E_MA
                   p_a_MB <- input$SI_p_a_E_MB
                   p_a_S <- input$SI_p_a_E_S 
                   p_b_MA <- input$SI_p_b_E_MA
                   p_b_MB <- input$SI_p_b_E_MB
                   p_b_S <- input$SI_p_b_E_S
                   
                   p_a_MA_s <- input$SI_p_a_SE_MA
                   p_a_MB_s <- input$SI_p_a_SE_MB
                   p_a_S_s <- input$SI_p_a_SE_S 
                   p_b_MA_s <- input$SI_p_b_SE_MA
                   p_b_MB_s <- input$SI_p_b_SE_MB
                   p_b_S_s <- input$SI_p_b_SE_S
...
c_E_h <- ((ru_E_h_icu*pp_E_h_icu*uc_h_icu)+(ru_E_h_noicu*pp_E_h_noicu*uc_h_noicu)+(ru_E_h_er*pp_E_h_er*uc_h_er))
c_E_o <- ((ru_E_o_pulmon*pp_E_o_pulmon*uc_o_pulmon)+(ru_E_o_gp*pp_E_o_gp*uc_o_gp)+(ru_E_o_anti*pp_E_o_anti*uc_o_anti)+(ru_E_o_syssteroid*pp_E_o_syssteroid*uc_o_syssteroid)+(ru_E_o_inhsteroid*pp_E_o_inhsteroid*uc_o_inhsteroid)+(ru_E_o_theoph*pp_E_o_theoph*uc_o_theoph))

c_SE_h <-((ru_SE_h_icu*pp_SE_h_icu*uc_h_icu)+(ru_SE_h_noicu*pp_SE_h_noicu*uc_h_noicu)+(ru_SE_h_er*pp_SE_h_er*uc_h_er))

c_SE_o <-((ru_SE_o_pulmon*pp_SE_o_pulmon*uc_o_pulmon)+(ru_SE_o_gp*pp_SE_o_gp*uc_o_gp)+(ru_SE_o_anti*pp_SE_o_anti*uc_o_anti)+(ru_SE_o_syssteroid*pp_SE_o_syssteroid*uc_o_syssteroid)+(ru_SE_o_inhsteroid*pp_SE_o_inhsteroid*uc_o_inhsteroid)+(ru_SE_o_theoph*pp_SE_o_theoph*uc_o_theoph)+(ru_SE_o_oxy*pp_SE_o_oxy*uc_o_oxy))

c_MA <-(((ru_MA_gp*pp_MA_gp*uc_outgp)+(ru_MA_spiro*pp_MA_spiro*uc_spiro)+(ru_MA_infvac*pp_MA_infvac*uc_infvac)+(ru_MA_beta*pp_MA_beta*uc_beta)+(ru_MA_theoph*pp_MA_theoph*uc_theoph)+(ru_MA_steroid*pp_MA_steroid*uc_steroid))/12)

c_MB <- (((ru_MB_gp*pp_MB_gp*uc_outspecialist)+(ru_MB_spiro*pp_MB_spiro*uc_spiro)+(ru_MB_infvac*pp_MB_infvac*uc_infvac)+(ru_MB_beta*pp_MB_beta*uc_beta)+(ru_MB_theoph*pp_MB_theoph*uc_theoph)+(ru_MB_steroid*pp_MB_steroid*uc_steroid))/12)

c_S <- (((ru_S_gp*pp_S_gp*uc_outspecialist)+(ru_S_spiro*pp_S_spiro*uc_spiro)+(ru_S_infvac*pp_S_infvac*uc_infvac)+(ru_S_beta*pp_S_beta*uc_beta)+(ru_S_theoph*pp_S_theoph*uc_theoph)+(ru_S_steroid*pp_S_steroid*uc_steroid))/12)

#drug cost

c_drug_trtA <- c_a*30.5*n_cycles*ppp
c_drug_trtB <- c_b*30.5*n_cycles*ppp

#hospitalized cost for ex_trtA
hc_ex_MA_trtA <- ((p_a_MA*(1-p_a_MA_s))*(c_E_h)*ppp)
hc_ex_MB_trtA <- ((p_a_MB*(1-p_a_MB_s))*(c_E_h)*ppp)                 
hc_ex_S_trtA <- ((p_a_S*(1-p_a_S_s))*(c_E_h)*ppp)                  

#other cost for ex_trtA
oc_ex_MA_trtA <- ((p_a_MA*(1-p_a_MA_s))*(c_E_o)*ppp)
oc_ex_MB_trtA <- ((p_a_MB*(1-p_a_MB_s))*(c_E_o)*ppp)                
oc_ex_S_trtA <- ((p_a_S*(1-p_a_S_s))*(c_E_o)*ppp)

#hospitalized cost for severe ex_trtA
hc_exs_MA_trtA <- ((p_a_MA*(p_a_MA_s))*(c_SE_h)*ppp)
hc_exs_MB_trtA <- ((p_a_MB*(p_a_MB_s))*(c_SE_h)*ppp)                 
hc_exs_S_trtA <- ((p_a_S*(p_a_S_s))*(c_SE_h)*ppp)                  

#other cost for severe ex_trtA
oc_exs_MA_trtA <- ((p_a_MA*(p_a_MA_s))*(c_SE_o)*ppp)
oc_exs_MB_trtA <- ((p_a_MB*(p_a_MB_s))*(c_SE_o)*ppp)                  
oc_exs_S_trtA  <- ((p_a_S*(p_a_S_s))*(c_SE_o)*ppp) 

#hospitalized cost for ex_trtB
hc_ex_MA_trtB <- ((p_b_MA*(1-p_b_MA_s))*(c_E_h)*ppp)
hc_ex_MB_trtB <- ((p_b_MB*(1-p_b_MB_s))*(c_E_h)*ppp)                  
hc_ex_S_trtB <- ((p_b_S*(1-p_b_S_s))*(c_E_h)*ppp)                 

#other cost for ex_trtB
oc_ex_MA_trtB <- ((p_b_MA*(1-p_b_MA_s))*(c_E_o)*ppp)
oc_ex_MB_trtB <- ((p_b_MB*(1-p_b_MB_s))*(c_E_o)*ppp)                 
oc_ex_S_trtB <- ((p_b_S*(1-p_b_S_s))*(c_E_o)*ppp) 

#hospitalized cost for severe ex_trtB
hc_exs_MA_trtB <- ((p_b_MA*(p_b_MA_s))*(c_SE_h)*ppp)
hc_exs_MB_trtB <- ((p_b_MB*(p_b_MB_s))*(c_SE_h)*ppp)                 
hc_exs_S_trtB <- ((p_b_S*(p_b_S_s))*(c_SE_h)*ppp)                 

#other cost for severe ex_trtB
oc_exs_MA_trtB <- ((p_b_MA*(p_b_MA_s))*(c_SE_o)*ppp)
oc_exs_MB_trtB <- ((p_b_MB*(p_b_MB_s))*(c_SE_o)*ppp)                 
oc_exs_S_trtB <- ((p_b_S*(p_b_S_s))*(c_SE_o)*ppp)
...
df_prob_trtA_first<-data.frame(a_P_trtA[,,1],row.names = c("MA","MB","S"))
df_prob_trtA_after<-data.frame(a_P_trtA[,,2],row.names = c("MA","MB","S"))
...
df_prob_trtB_first<-data.frame(a_P_trtB[,,1],row.names = c("MA","MB","S"))
df_prob_trtB_after<-data.frame(a_P_trtB[,,2],row.names = c("MA","MB","S"))
